    ...
    if( params ) {
        ...
        for( int i = 0; i < param_n; ++i ) {
            ...
            if( !strcmp( item_name->valuestring, "Albedo" ) ) {
                ...
                else if( 2 == item_value->valueint )
                    s->proj_matrix_type = EGui;
                else
                    s->proj_matrix_type = ENone;
            }
        }
    } else {
        s->proj_matrix_type = ENone;
...
int LoadMesh( const char *pFile ) {
    ...
    handle = Renderer_createStaticMesh( ETriangles, indices_v, indice_n * sizeof( u32 ), positions_v, position_n * sizeof( vec2 ), texcoords_v, position_n * sizeof( vec2 ) );
